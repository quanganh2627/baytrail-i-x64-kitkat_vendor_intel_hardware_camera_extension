...
typedef enum intel_camera_metadata_section_start {
  % for i in find_all_sections(metadata):
    ${path_name(i) + '.start' | csym,ljust(30)} = (${path_name(i) | csym,pad(64)} << 16 ) | VENDOR_SECTION_START,
  % endfor
...
% for sec in find_all_sections(metadata):
  % for entry in remove_synthetic(find_unique_entries(sec)):
    % if entry.enum:
// ${entry.name | csym}
typedef enum intel_camera_metadata_enum_${csym(entry.name).lower()} {
    ...
    ${'%s_%s'%(csym(entry.name), val.name) | pad(65)} = ${val.id},
        % endif
      % endfor
} intel_camera_metadata_enum_${csym(entry.name).lower()}_t;
