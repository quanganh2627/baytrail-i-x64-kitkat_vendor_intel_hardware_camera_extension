...
{
    LOGI("%s", __FUNCTION__);
    CP_cpu_hdrUninit(env, thiz, instance);
    CP_cpu_ullUninit(env, thiz, instance);
...
jint CP_cpu_hdrInit(JNIEnv* env, jobject thiz, jlong instance, jint width, jint height, jobject jBlendOption)
{
    LOGI("%s", __FUNCTION__);
    CPEngine* cpe = getCpEngine(instance);
    ...
    if (tgt != -1)
        err = ia_cp_hdr_init(&cpe->pIaCpHdr, ctx, width, height, &aiqb_data, tgt_lut[tgt]);
    else {
        LOGE("blend option error!");
    }
//  int i;
//  for (i = 0 ; i < 1024 ; i++)
//  {
...
jobject CP_cpu_hdrCompose(JNIEnv* env, jobject thiz, jlong instance, jobjectArray jInputIaFrames, jobject jHdrOption)
{
    LOGI("%s", __FUNCTION__);
...
jint CP_cpu_ullInit(JNIEnv* env, jobject thiz, jlong instance, jint width, jint height, jobject jBlendOption)
{
    LOGI("%s", __FUNCTION__);
    CPEngine* cpe = getCpEngine(instance);
    ia_cp_context* ctx = getCpContext(instance);
    ...
    if (jBlendOption != NULL) {
        ...
        if (tgt != -1)
            ...
            LOGE("get Blend Option Failed!");
    } else {
        LOGD("blend option is null!");
    }
    ia_err err = ia_err_none;
    if (tgt != -1)
        err = ia_cp_ull_init(&cpe->pIaCpUll, ctx, width, height, &aiqb_data, tgt_lut[tgt]);
    else
        LOGE("Blend Option error!");
    LOGD("ret = %d", err);
    return err;
...
jobject CP_cpu_ullCompose(JNIEnv* env, jobject thiz, jlong instance, jobjectArray jInputIaFrames, jobject jUllOption)
{
    LOGI("%s", __FUNCTION__);
